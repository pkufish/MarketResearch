...
# 全局配置参数
MIN_GROWTH_RATE = 20  # 最小增长率要求（百分比）
MIN_DAYS = 5         # 最小天数
MAX_DAYS = 60        # 最大天数
TOP_N = 10           # 显示组数
TARGET = 'QQQ' # 标的
...
def find_top_growth_intervals(data):
    """
    查找前N组最快增长区间，确保区间之间不重叠
    """
    growth_intervals = []
    ...
    # 按增长率排序
    growth_intervals.sort(key=lambda x: x['growth_rate'], reverse=True)
    
    # 选择不重叠的前N个区间
    selected_intervals = []
    for interval in growth_intervals:
...
try:
    ...
    # 查找前10组最快增长区间
    top_growth_intervals = find_top_growth_intervals(data)

    # 打印前10组最快增长信息
    print(f"\n前{TOP_N}组最快增长分析（增长率>{MIN_GROWTH_RATE}%）:")
    for i, interval in enumerate(top_growth_intervals, 1):
        print(f"\n第{i}组:")
        print(f"最大增长率: {interval['growth_rate']:.2f}%")
        print(f"增长天数: {interval['days']}天")
    ...
    # 保存分析数据
    if saved_file:
        analysis_file = saved_file.replace('.csv', '_top_growth.csv')
    ...
    # 使用不同颜色绘制前10组增长区间
    colors = plt.cm.rainbow(np.linspace(0, 1, len(top_growth_intervals)))
    for i, interval in enumerate(top_growth_intervals):
        plt.plot([interval['start_date'], interval['end_date']], 
                 [interval['start_price'], interval['end_price']], 
                 color=colors[i], linewidth=2, 
                 label=f'第{i+1}组: {interval["growth_rate"]:.2f}%\n'
    ...
    plt.title(f'{ticker} 前{TOP_N}组最快增长分析（增长率>{MIN_GROWTH_RATE}%）')
    plt.xlabel('日期')
    plt.ylabel('价格 ($)')
